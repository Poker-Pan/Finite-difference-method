...
id_repeapt = [x * (Num_y+1) for x in range(0, (Num_x+1))]
id_bounary_corner = list([1, Num_x, Num_all-1-(Num_x-1), Num_all-1])
...
id_bounary_time = list(set(list(set(id_bounary_time_initial).union(id_bounary_time_end))).difference(id_repeapt))
id_bounary_space = list(set(list(set(list(np.where(np.logical_or(nodes[:, 1] == y[0]+h_y, nodes[:, 1] == y[1]))[0])).difference(id_bounary_corner))).difference(id_repeapt))
