...
# 8   0.23369036257546805       0.23429398725610517
# 16  0.05302390211423469       0.053099340545600615
# 32  0.012950206533824282      0.012959518965309336
# 48
...
id_bounary_corner = list([(Num_x+1)+1, (Num_x+1)+1 + (Num_x-1), ((Num_x+1)*(Num_y+1))-1 - (Num_x-1), ((Num_x+1)*(Num_y+1))-1,
                          ((Num_x+1)*(Num_y+1))+(Num_x+1)+1 + ((Num_x+1)*(Num_y+1)*(Num_z-1)), ((Num_x+1)*(Num_y+1))+(Num_x+1)+1 + (Num_x-1) + ((Num_x+1)*(Num_y+1)*(Num_z-1)), ((Num_x+1)*(Num_y+1)*(Num_z+1)) - 1 - (Num_x-1), ((Num_x+1)*(Num_y+1)*(Num_z+1)) - 1])
id_repeapt = list(set(list(np.where(nodes[:, 1] == y[0])[0])).union(list(np.where(nodes[:, 2] == z[0])[0])))

id_bounary_time_initial = list(set(list(set(list(np.where(nodes[:, 0] == x[0])[0])).difference(id_bounary_corner))).difference(id_repeapt))
id_bounary_time_end = list(set(list(set(list(np.where(nodes[:, 0] == x[1])[0])).difference(id_bounary_corner))).difference(id_repeapt))
id_bounary_time = list(set(id_bounary_time_initial).union(id_bounary_time_end))
id_bounary_space = list(set(list(set(list(set(list(np.where(np.logical_or(np.logical_or(nodes[:, 1] == y[0]+h_y, nodes[:, 1] == y[1]),
                                                                 np.logical_or(nodes[:, 2] == z[0]+h_z, nodes[:, 2] == z[1])))[0])).difference(id_bounary_time))).difference(id_bounary_corner))).difference(id_repeapt))

id_external = list(set(list(set(list(set(id_bounary_time).union(id_bounary_space))).union(id_bounary_corner))).difference(id_repeapt))
id_internal = list(set(list(set(id_all).difference(id_external))).difference(id_repeapt))
...
for i in range(len(id_bounary_time_initial)):
    ...
    if cur_nodes[1] == y[0]+h_y:
        ...
        A[id_bounary_time_initial[i], id_bounary_time_initial[i] + 1] = -1 / (h_x * h_x)
        A[id_bounary_time_initial[i], id_bounary_time_initial[i] + (Num_x + 1)] = -1 / (h_y * h_y)
        A[id_bounary_time_initial[i], id_bounary_time_initial[i] + (Num_x + 1)*(Num_y - 1)] = -1 / (h_y * h_y)
...
for i in range(len(id_bounary_time_end)):
    ...
    if cur_nodes[1] == y[0]+h_y:
        ...
        A[id_bounary_time_end[i], id_bounary_time_end[i] + 1] = -1 / (h_x * h_x)
        A[id_bounary_time_end[i], id_bounary_time_end[i] + (Num_x + 1)] = -1 / (h_y * h_y)
        A[id_bounary_time_end[i], id_bounary_time_end[i] + (Num_x + 1)*(Num_y - 1)] = -1 / (h_y * h_y)
    elif cur_nodes[2] == z[1]:
        A[id_bounary_time_end[i], id_bounary_time_end[i] - 1] = -1 / (h_x * h_x)
        A[id_bounary_time_end[i], id_bounary_time_end[i] + 1] = -1 / (h_x * h_x)
        A[id_bounary_time_end[i], id_bounary_time_end[i] - (Num_x + 1)] = -1 / (h_y * h_y)
...
for i in range(len(id_bounary_space)):
    ...
    if cur_nodes[1] == y[0]+h_y and cur_nodes[2] != z[0]+h_z and cur_nodes[2] != z[1]:
        ...
        A[id_bounary_space[i], id_bounary_space[i] + 1] = -1 / (h_x * h_x)
        A[id_bounary_space[i], id_bounary_space[i] + (Num_x + 1)] = -1 / (h_y * h_y)
        A[id_bounary_space[i], id_bounary_space[i] + (Num_x + 1)*(Num_y - 1)] = -1 / (h_y * h_y)
...
for i in range(len(id_bounary_corner)):
    ...
    if cur_nodes[0] == x[0]:
        ...
        if cur_nodes[1] == y[0]+h_y and cur_nodes[2] == z[0]+h_z:
            ...
            A[id_bounary_corner[i], id_bounary_corner[i] + (Num_x + 1)] = -1 / (h_y * h_y)
            A[id_bounary_corner[i], id_bounary_corner[i] + (Num_x + 1) * (Num_y - 1)] = -1 / (h_y * h_y)
        elif cur_nodes[1] == y[1] and cur_nodes[2] == z[0]+h_z:
            A[id_bounary_corner[i], id_bounary_corner[i] - 1] = -1 / (h_x * h_x)
            A[id_bounary_corner[i], id_bounary_corner[i] - (Num_x - 1)] = -1 / (h_x * h_x)
            A[id_bounary_corner[i], id_bounary_corner[i] + (Num_x + 1)] = -1 / (h_y * h_y)
...
for i in range(len(id_repeapt)):
    cur_nodes = nodes[id_repeapt[i], :]
    if cur_nodes[1] == y[0] and cur_nodes[2] == z[0]:
        A[id_repeapt[i], id_repeapt[i]] = 1
        A[id_repeapt[i], id_repeapt[i] + (Num_x + 1)*(Num_y + 1)-1] = -1
    elif cur_nodes[1] == y[0]:
        A[id_repeapt[i], id_repeapt[i]] = 1
        A[id_repeapt[i], id_repeapt[i] + (Num_x)] = -1
    elif cur_nodes[2] == z[0]:
        A[id_repeapt[i], id_repeapt[i]] = 1
        A[id_repeapt[i], id_repeapt[i] + (Num_x + 1)*(Num_y)] = -1
