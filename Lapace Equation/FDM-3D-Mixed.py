...
# 8   0.23474532679359883   0.24058600828576293
# 16  0.053136553305975776  0.05387217709783787
# 32  0.012970535890014512  0.013116226723100421
...
def N_bounary(nodes):
    ...
    for i in range(nodes.shape[0]):
        ...
        if cur_x == x[0]:
            if cur_y == y[0]:
                out[i] = f(cur_x, cur_y, cur_z) - 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_z - 2*(np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_x
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_z - 2*(-np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_x
            elif cur_y == y[1]:
                out[i] = f(cur_x, cur_y, cur_z) - 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_z + 2*(np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_x
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_z + 2*(-np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_x
            elif cur_z == z[0]:
                out[i] = f(cur_x, cur_y, cur_z) - 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_z - 2*(np.sin(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_y
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_z - 2*(-np.pi*np.cos(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_y
            elif cur_z == z[1]:
                out[i] = f(cur_x, cur_y, cur_z) - 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_z + 2*(np.sin(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_y
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_z + 2*(-np.pi*np.cos(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_y
            elif cur_y == y[0] and cur_z == z[0]:
                out[i] = f(cur_x, cur_y, cur_z) - 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_z - 2*(np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_x - 2*(np.sin(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_y
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_z - 2*(-np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_x - 2*(-np.pi*np.cos(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_y
            ...
            if cur_y == y[0] and cur_z != z[0] and cur_z != z[1]:
                ...
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_x
                ...
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.cos(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_y
                ...
                # out[i] = f(cur_x, cur_y, cur_z) - 2*(-np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_x - 2*(-np.pi*np.cos(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_y
            elif cur_y == y[1] and cur_z == z[0]:
                out[i] = f(cur_x, cur_y, cur_z) + 2*(np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_x - 2*(np.pi*np.sin(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_y
                # out[i] = f(cur_x, cur_y, cur_z) + 2*(-np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)*np.cos(np.pi*cur_z))/h_x - 2*(-np.pi*np.cos(np.pi*cur_x)*np.cos(np.pi*cur_y)*np.sin(np.pi*cur_z))/h_y
            elif cur_y == y[0] and cur_z == z[1]:
