...
# 16  0.15364296928079887     0.05302928754551539
# 32  0.04018443032858345     0.01295075348262853
# 64  0.010150661686001747    0.00321904714844945
...
def N_bounary(nodes):
    ...
    for i in range(nodes.shape[0]):
        ...
        cur_y = nodes[i, 1]
        temp = nodes[i]
        if temp[1] == y[0] and temp[0] != x[0] and temp[0] != x[1]:
            ...
            out[i] = f(cur_x, cur_y) - 2*(cur_x*(cur_x-1)*(2*cur_y-1)) / h_y
            ...
            out[i] = f(cur_x, cur_y) - 2*((2*cur_x-1)*cur_y*(cur_y-1)) / h_x
        elif temp[0] == x[1] and temp[1] != y[0] and temp[1] != y[1]:
            # out[i] = f(cur_x, cur_y) + 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)) / h_x
            # out[i] = (f(cur_x, cur_y) + 2 * (-np.pi * np.sin(np.pi * cur_x) * np.cos(np.pi * cur_y)) / h_x)
            out[i] = f(cur_x, cur_y) + 2*((2*cur_x-1)*cur_y*(cur_y-1)) / h_x
        elif temp[0] == x[0] and temp[1] == y[0]:
            # out[i] = f(cur_x, cur_y) - 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)) / h_x - 2*(np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)) / h_y
            # out[i] = (f(cur_x, cur_y) - 2 * (-np.pi * np.sin(np.pi * cur_x) * np.cos(np.pi * cur_y)) / h_x - 2 * (-np.pi * np.cos(np.pi * cur_x) * np.sin(np.pi * cur_y)) / h_y)
            out[i] = f(cur_x, cur_y) - 2*((2*cur_x-1)*cur_y*(cur_y-1)) / h_x - 2*(cur_x*(cur_x-1)*(2*cur_y-1)) / h_y
        elif temp[0] == x[0] and temp[1] == y[1]:
            # out[i] = f(cur_x, cur_y) - 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)) / h_x + 2*(np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)) / h_y
            # out[i] = (f(cur_x, cur_y) - 2 * (-np.pi * np.sin(np.pi * cur_x) * np.cos(np.pi * cur_y)) / h_x + 2 * (-np.pi * np.cos(np.pi * cur_x) * np.sin(np.pi * cur_y)) / h_y)
            out[i] = f(cur_x, cur_y) - 2*((2*cur_x-1)*cur_y*(cur_y-1)) / h_x + 2*(cur_x*(cur_x-1)*(2*cur_y-1)) / h_y
        elif temp[0] == x[1] and temp[1] == y[0]:
            # out[i] = f(cur_x, cur_y) + 2*(np.pi*np.cos(np.pi*cur_x)*np.sin(np.pi*cur_y)) / h_x - 2*(np.pi*np.sin(np.pi*cur_x)*np.cos(np.pi*cur_y)) / h_y
            # out[i] = (f(cur_x, cur_y) + 2 * (-np.pi * np.sin(np.pi * cur_x) * np.cos(np.pi * cur_y)) / h_x - 2 * (-np.pi * np.cos(np.pi * cur_x) * np.sin(np.pi * cur_y)) / h_y)
            out[i] = f(cur_x, cur_y) + 2*((2*cur_x-1)*cur_y*(cur_y-1)) / h_x - 2*(cur_x*(cur_x-1)*(2*cur_y-1)) / h_y
